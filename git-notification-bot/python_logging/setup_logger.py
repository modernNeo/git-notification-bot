...
class BotWarnStreamHandler(logging.StreamHandler):
    def emit(self, record):
        if record.levelno < error_logging_level:
            super().emit(record)


class BotDebugStreamHandler(logging.StreamHandler):
    def emit(self, record):
...
class Loggers:
    ...
    def _setup_sys_logger(cls):
        ...
        sys_stdout_stream_handler = BotDebugStreamHandler(sys.stdout)
        ...
        sys_std_warn_stream_handler = BotWarnStreamHandler(sys.stdout)
